...
function SpawnCreepsOnPoints(creep_table)
	...
	if hero_max_level > 7 then -- old 10
		count_min = 2
		count_max = 5
		max_level = 3 -- level 10
	end

	if hero_max_level > 12 then -- old 15
		count_min = 3
		count_max = 6
...
function IsBossAlive(unit_table, unit_name)
	for _, x in pairs(unit_table) do
		if x.unit then
			if x.unit:GetUnitName() == unit_name then
				return true
			end
		end
	end
...
function  GetRandomItemFromTable(table)
	if not table then return end
	local rand, j, tries
	rand = RandomInt(1, GetTableSize(table))
	j = 1
...
function DropItem(unit)
	...
	if not drop_table[unit:GetUnitName()] then
		return
	end
	...
	for i = 1, max_item_count do 
		if not IsUnitBossGlobal(unit) then
			item = GetRandomItemFromTable(drop_table[unit:GetUnitName()])
		else
